...
Returning to our example agent, we now add slightly more information, and stipulate that they have in fact encountered a harshly judgemental midwife in all three rounds, hence \(n_{h}=3\). Their prior beliefs remain uninformative, i.e. \(\alpha_{j} = 1, j \in \{l,m,h\}\), \(\alpha_{i}=1,i \in \{r,n\}\). As before, the agent evaluates \(R_{w}\) for the three signals, and the process for the light signal is given below -

\begin{equation*}
\begin{align*}
R_{w}(s_{w}, l) &= \sum_{i\in A_{m}}\sum_{j\in \Theta} -u_{w}(s_{w}, i, l, j)p(j)p(i | s_{w}),
R_{w}(s_{w}, l) &= 
\end{align}
\end{equation}
