...
At the end of a game, each player receives a payoff dependent on the actions and types of both players, which has a partially common interest component. Women receive a payoff based on the health of their eventual baby, with a social cost dependent on the signal they sent and the midwife's reaction to it. Midwives receive the same health payoff as the women, but pay a cost for referring to a specialist, mirroring the organisational cost of non-routine care (table \ref{tab:Payoff-matrix}).
...
Midwives play for \(A_{m}\) rounds (\(A_{m}=1000\) in all experiments), and conduct appointments in parallel, i.e. if there are 5 midwives, then five women are drawn from the queue and assigned at random to the midwives. 
...
\begin{table}
\center
\subfloat[Social cost, \(P_{s}\)\label{tab:Social-cost-matrix}]{%
...
\subfloat[Health outcome, \(P_{h}\)\label{tab:Referral-payoff-matrix}]{%
\begin{tabular}{|c|c|c|c|c|}
\cline{3-5} 
\multicolumn{2}{c}{} & \multicolumn{3}{|c|}{Woman}\tabularnewline
...
\subfloat[Referral cost, \(P_{c}\)\label{tab:inst_cost_matrix}]{%
\begin{tabular}{|c|c|c|c|c|}
\cline{3-5}  
\multicolumn{2}{c}{} & \multicolumn{3}{|c|}{Woman}\tabularnewline
\hline 
...
Formally we let \(S\) be the set of signals \(\{l, m, h\}\), \(T_{w}\) be the set of drinking types \(\{l, m, h\}\), \(T_{m}\) the set of midwife types \(\{l, m, h\}\), and \(J=\{r,n\}\) the actions available to midwives. The set of all payoffs shall be denoted \(P\), where \(P_{w}\) are the payoffs for all women\footnote{While it is assumed that \(P_{w}\) for an individual of type \(a\) is constrained such that \(T=\{a\}\), this can change under the modified game described in subsection \ref{sub:info_sharing}} defined by \(P_{w}=\{x | x=P_{h, b, c} + P_{s, a, d}\wedge b \in J\wedge c \in T\wedge d \in M\}\), and equivalently for midwives \(P_{m}=\{x | x=P_{h, b, c} + P_{c, b}\}\).


\subsection{Lexicographic Heuristic}
\label{sub:lexico}

The lexicographic heuristic (algorithm \ref{alg:lexico}) follows the form of that used in \cite{Hau2008}, and assumes a simplified decision problem, as in figure \ref{fig:payoff_problem}, where an action is a choice between combined lotteries. Functionally, the heuristic maintains a count of the number of times that each action was followed by a payoff, and chooses the action which most commonly has the best payoff, i.e. one reason decision making. This approach requires minimal computation, and does not assume that all payoffs are known, or static.

Women maintain a count, \(n\) for each of the \(k\) payoffs in \(P_{w, a},a\in k\), an individual payoff paired with a signal, such that \(n_{i}=\{n_{i,1}\ldots n_{i,k}\}\).

Midwives solve a slightly different problem, where the signal received may convey useful information. This is resolved by maintaining a separate count for each signal, i.e. \(n_{i,j}=\{n_{i,j,1}\ldots n_{i, j, k}\}\), and otherwise follow the same algorithm.
...
The second Bayesian agent augments the payoff reasoning version by separating the two lotteries and assuming that \(P\) is a static, and known set of payoffs possible for the agent. For women, this means that they maintain two sets of beliefs: a Dirichlet mixture which represents the believed distribution of midwife types, and a second set linking signals to referrals.
