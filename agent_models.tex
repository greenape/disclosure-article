...
simulation, the Dirichlet distribution is employed as a prior to represent these beliefs \citep{Agresti2005}. 
...
Where \(n_{j}\) is simply the count of occurrences of signal-payoff pair \(j\), and \(\alpha_{j}\) is the pseudo-count of prior observations\footnote{Psuedo-counts are related to, but distinct from prior beliefs. Here, the pseudo-count is a hyperparameter of the prior Dirichlet distribution and is nothing more than a hypothetical count of prior observations. The outcome is a result of the relationship between $\alpha$, and $n$, which is increasingly driven by the priors as for higher values of $\alpha$.} for a pair \(j\). Hence,
...
\begin{equation}
R_{w}(s_{w}, \theta_{w}) = \sum_{i\in A_{m}}\sum_{j\in \Theta} -u_{w}(s_{w}, i, \theta_{w}, j)p(j)p(i | s_{w})\label{eq:w_brm},
\end{equation}
...
\begin{equation}
R_{m}(s_{w}, s_{m}) = \sum_{i\in \Theta} -u_{m}(s_{w}, s_{m}, i)p(i | s_{w})\label{eq:mw_brm}
\end{equation}
...
\end{equation}
\begin{equation}
\pi_{i}^{-}=w^{-}(p_{-m}+\ldots+p_{i}) - w^{-}(p_{-m}+\ldots+p_{i-1}),1-m\leq i \leq 0\label{eq:cpt_end}
\end{equation}
